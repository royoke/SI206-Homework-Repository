...
# Section Day/Time: SI 206 Section 005 Thursday/6:00pm - 7:00pm
# People you worked with: Cameron Chandra
...
def string_manip(s):
    return "".join(reversed(s.strip().upper().replace(" ","#").replace("UMSI","")))
...
def name_counts(names):
    ...
    name_list = sorted(name_dict.items())
    return sorted(name_list, key = lambda x: x[1], reverse = True)
...
def build_acronym(ls):
    ...
    for string in ls:
        ...
        acro_list.append(string)
    return "".join(acro_list)
